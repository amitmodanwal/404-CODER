...
import { ArrowRight, CheckCircle2, AlertTriangle, Calendar, Award, BookOpen, FileText } from 'lucide-react';
...
export const ProfileConfirmation: React.FC<ProfileConfirmationProps> = ({
  ...
  const { member, signals, missions, resumeText } = candidate as Candidate & { resumeText?: string | null };
    ...
    <div style={{ maxWidth: '1100px', margin: '36px auto', padding: '0 20px' }}>
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ fontSize: '14.5px', color: 'var(--text-secondary)' }}>
          Review candidate profile, extracted resume text, and verified 31-day cohort learning signals.
        </p>
      </div>
      ...
      >
        ...
        <Card accentBorder style={{ position: 'sticky', top: '90px' }}>
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            ...
            >
              ...
              <img
                src={member.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=250&q=80'}
                alt={member.name}
        ...
        {/* Right: Learning Journey & Extracted Resume Excerpt */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {/* Extracted Resume Excerpt Callout (if present) */}
          {resumeText && (
            <Card accentBorder style={{ backgroundColor: 'rgba(110, 91, 255, 0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', color: 'var(--accent-primary)' }}>
                <FileText size={18} />
                <h4 style={{ fontSize: '15px', fontWeight: 700 }}>Extracted Resume Excerpt</h4>
              </div>
              ...
              <span style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'inline-block', marginTop: '6px' }}>
                ✓ Resume text will be passed to the AI Interview Agent to generate tailored questions.
              </span>
            </Card>
          )}
          ...
          <Card>
            ...
            {completedMissions.length === 0 ? (
              <p style={{ fontSize: '13.5px', color: 'var(--text-muted)' }}>
                No mission history yet — this interview will be based on your live answers only.
              </p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {curriculum.modules.map((mod) => {
                  ...
                  return (
                    <div key={mod.id} style={{ borderBottom: '1px solid var(--border-subtle)', paddingBottom: '14px' }}>
