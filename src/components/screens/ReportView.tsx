...
export const ReportView: React.FC<ReportViewProps> = ({
  ...
  const categories = report.categoryBreakdown.map((category) => ({
    ...category,
    moduleTitle: category.moduleTitle || (category as unknown as { module?: string }).module || 'Untitled module',
  }));
  const sortedCategories = [...categories].sort((a, b) => b.score - a.score);
  const strongestAreas = sortedCategories.filter((category) => category.score >= 60).slice(0, 2);
  const weakestAreas = [...sortedCategories].reverse().slice(0, 2);
  ...
  return (
    <div className="print-report" style={{ maxWidth: '1100px', margin: '36px auto', padding: '0 20px' }}>
      {/* Header Banner */}
      <Card accentBorder padding="28px 32px" style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '24px' }}>
          ...
          <div className="report-actions" style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '24px', alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Card padding="20px">
            <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '12px' }}>Strongest Areas</h3>
            {strongestAreas.length ? strongestAreas.map((area) => (
              <p key={area.moduleTitle} style={{ fontSize: '13px', color: 'var(--text-primary)', marginBottom: '8px' }}>{area.moduleTitle} <strong style={{ color: 'var(--accent-success)' }}>{area.score}%</strong></p>
            )) : <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>No strong areas identified this session.</p>}
            <h3 style={{ fontSize: '16px', fontWeight: 700, margin: '18px 0 12px' }}>Weakest Areas</h3>
            {weakestAreas.map((area) => (
              <p key={area.moduleTitle} style={{ fontSize: '13px', color: 'var(--text-primary)', marginBottom: '8px' }}>{area.moduleTitle} <strong style={{ color: 'var(--accent-danger)' }}>{area.score}%</strong></p>
            ))}
          </Card>
