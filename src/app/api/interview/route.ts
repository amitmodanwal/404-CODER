...
interface ServerSessionState {
  ...
  answers: { questionId: string; questionText: string; answerText: string; isFollowUp: boolean; dayNumber: number }[];
  daysCovered: number[];
  targetCount: number;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'respond' && answer) {
      ...
      const shouldFollowUp = !lastQuestion?.isFollowUp && totalAsked < session.targetCount - 1 && Math.random() > 0.4;
...
function generateFinalFeedbackResponse(session: ServerSessionState) {
  ...
  const avgAnswerLength = answeredCount > 0
    ? answers.reduce((acc, a) => acc + a.answerText.length, 0) / answeredCount
    : 0;

  // Base score calculation from candidate signals + answer depth
  let overallScore = Math.min(96, Math.max(68, Math.round(candidate.signals.passRate * 85 + (avgAnswerLength > 100 ? 12 : 5))));
  if (difficulty === 'Advanced') overallScore = Math.min(98, overallScore + 3);
  ...
  const categoryBreakdown = touchedModuleTitles.map(modTitle => ({
    ...
    score: Math.min(98, Math.max(65, Math.round(overallScore + (Math.random() * 14 - 7)))),
    questionsCount: Math.max(1, Math.floor(session.questionsAsked.length / touchedModuleTitles.length)),
  }));
